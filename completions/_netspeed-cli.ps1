...
Register-ArgumentCompleter -Native -CommandName 'netspeed-cli' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'netspeed-cli' {
            ...
            [CompletionResult]::new('--no-color', '--no-color', [CompletionResultType]::ParameterName, 'Disable all colors (equivalent to NO_COLOR=1)')
            [CompletionResult]::new('--no-emoji', '--no-emoji', [CompletionResultType]::ParameterName, 'Replace emoji/icons with plain text (for terminals without emoji support)')
